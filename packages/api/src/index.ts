...
try {
  ...
  if (err?.position) {
    console.error('[STARTUP FAILED] position  :', JSON.stringify(err.position));
  }
  if (err?.specifier) {
    console.error('[STARTUP FAILED] specifier :', err.specifier);
  }
  if (err?.referrer) {
    console.error('[STARTUP FAILED] referrer  :', err.referrer);
  }
  process.exit(1);
}
